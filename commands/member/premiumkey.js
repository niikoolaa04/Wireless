...
module.exports = class PremiumKey extends Command {
  ...
  async run(message, args) {
    ...
    if(!member.roles.cache.some(r => r == this.client.config.developer.patreon)) 
      return message.channel.send({ embeds: [ this.client.embedBuilder(this.client, message.author, `Error`, `You aren't our Patreon or have left Support Server.`, "RED")] });
    if(generated == 1) 
      return message.channel.send({ embeds: [ this.client.embedBuilder(this.client, message.author, `Error`, `You have already generated maximum number of keys.`, "RED")] });
    ...
    await User.findOneAndUpdate({ id: message.author.id }, { $inc: { generateCount: 1 }}, { new: true, upsert: true });
  }
  async slashRun(interaction, args) {
    ...
    if(!member.roles.cache.some(r => r == this.client.config.developer.patreon)) 
      return interaction.reply({ embeds: [ this.client.embedBuilder(this.client, interaction.user, `Error`, `You aren't our Patreon or have left Support Server.`, "RED")], ephemeral: true });
    if(generated == 1) 
      return interaction.reply({ embeds: [ this.client.embedBuilder(this.client, interaction.user, `Error`, `You have already generated key.`, "RED")], ephemeral: true });
